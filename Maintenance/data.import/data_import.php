...
$arIBlockAvailProdFields = array(
    ...
    "IE_ELEMENT_META_TITLE" => array(
        "field" => "ELEMENT_META_TITLE",
        "important" => "N",
        "name" => "SEO Title (B_IBLOCK_ELEMENT.ELEMENT_META_TITLE)",
    ) ,
    "IE_ELEMENT_META_DESCRIPTION" => array(
        "field" => "ELEMENT_META_DESCRIPTION",
        "important" => "N",
        "name" => "SEO Description (B_IBLOCK_ELEMENT.ELEMENT_META_DESCRIPTION)",
    ) ,
    "IE_ELEMENT_META_KEYWORDS" => array(
        "field" => "ELEMENT_META_KEYWORDS",
        "important" => "N",
        "name" => "SEO Keywords (B_IBLOCK_ELEMENT.ELEMENT_META_KEYWORDS)",
    )
);
            ...
            while ($arRes = $csvFile->FetchAssoc())
            {
                ...
                if (strlen($strErrorR) <= 0)
                {
                    ...
                    foreach ($arRes as $key => $value)
                    {
                        ...
                        /*SEO*/
                        if (strncmp($key, "IE_ELEMENT_META", 7) == 0)
                        {
                            if ($key == "IE_ELEMENT_META_TITLE") {
                                $arLoadProductArray["IPROPERTY_TEMPLATES"]["ELEMENT_META_TITLE"] = $value;
                            }
                            if ($key == "IE_ELEMENT_META_DESCRIPTION") {
                                $arLoadProductArray["IPROPERTY_TEMPLATES"]["ELEMENT_META_DESCRIPTION"] = $value;
                            }
                            if ($key == "IE_ELEMENT_META_KEYWORDS") {
                                $arLoadProductArray["IPROPERTY_TEMPLATES"]["ELEMENT_META_KEYWORDS"] = $value;
                            }
                        }
                        /*SEO*/
                    }

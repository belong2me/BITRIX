...
EventManager::getInstance()->addEventHandler("iblock", "OnIBlockPropertyBuildList", array("PriceListProp", "GetIBlockPropertyDescription"));
...
class PriceListProp
{
    ...
    private static function getStyles()
    {
        if (!self::$stylesSet) {
            self::$stylesSet = true;
            Asset::getInstance()->addString("<style>" . file_get_contents(__DIR__ . '/price.list.css') . "</style>");
        }
        return '';
    }
    ...
    private static function getScripts($prop_id)
    {
        if (!self::$scriptsSet) {
            ...
            Asset::getInstance()->addString("<script>" . str_replace(
                    ['#PROP_ID#', '#URL#'],
                    [$prop_id, $url],
                    file_get_contents(__DIR__ . '/price.list.js')) . "</script>");
        }
        return '';
    }

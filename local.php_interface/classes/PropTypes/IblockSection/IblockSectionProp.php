...
namespace Rayonnant\PropTypes\IblockSection;
...
class IblockSectionProp {
    ...
    public static function GetUserTypeDescription() {
        return array(
            'PROPERTY_TYPE' => 'G',
            'DISPLAY_TYPE' => 'F',
            'USER_TYPE' => 'SASDSectionCustom',
            'DESCRIPTION' => Loc::getMessage('CUSTOM_PROP_IBP_SECTION_DESCR'),
        ...
        if (isset($value['VALUE']) && !empty($value['VALUE']) && !empty($arProperty['IBLOCK_ID'])) {
            ...
            if (isset(self::$cache[$value['VALUE']])) {
                if (self::$cache[$value['VALUE']]['IBLOCK_ID'] != $arProperty['IBLOCK_ID']) {
                    $arResult[] = GetMessage('CUSTOM_PROP_IBP_SECTION_NO_PARENT_IBLOCK');
                }
            }
        }
        return $arResult;
    ...
    public static function GetPropertyFieldHtml($arProperty, $arValue, $strHTMLControlName) {
        ...
        if (
            ...
            <select name="<? echo $strHTMLControlName["VALUE"]; ?>" id="<? echo $strHTMLControlName["VALUE"]; ?>">
                <option value=""><? echo GetMessage('CUSTOM_PROP_IBP_SECTION_MESS_TOP_LEVEL'); ?></option>
                <?
    ...
    public static function GetSettingsHTML($arFields, $strHTMLControlName, &$arPropertyFields) {
        $arPropertyFields = array(
            'USER_TYPE_SETTINGS_TITLE' => GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_TITLE'),
        );
        $arSettings = self::PrepareSettings($arFields);
        return '<tr>
		<td>'.GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_VIEW_MODE').'</td>
		<td>'.SelectBoxFromArray(
                $strHTMLControlName['NAME'].'[VIEW_MODE]',
                array(
                    'REFERENCE' => array(
                        GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_VIEW_MODE_SELECT'),
                        GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_VIEW_MODE_WINDOW')
                    ),
                    'REFERENCE_ID' => array(
		...
		<tr>
		<td>'.GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_MAX_LEVEL').'</td>
		<td><input type="text" name="'.$strHTMLControlName['NAME'].'[MAX_LEVEL]" value="'.$arSettings['MAX_LEVEL'].'"> '.GetMessage('CUSTOM_PROP_IBP_SECTION_SETTING_MAX_LEVEL_DESCR').'</td>
		</tr>';
    }

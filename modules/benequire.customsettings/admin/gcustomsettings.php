...
$module_mode = CModule::IncludeModuleEx("benequire.customsettings");
...
//require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data.php");

IncludeModuleLangFile(__FILE__);

require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/prolog.php");
	...
	foreach($arCustomSettings as $tab_id => $arTab) {
        ...
        foreach ($arTab["FIELDS"] as $arField) {
            ...
            if ($arField["TYPE"] == 'file') {
                ...
                if (!is_array(${$arField["NAME"]}) && !empty(${$arField["NAME"]}) && isset(${$arField["NAME"].'_del'}) && ${$arField["NAME"].'_del'} == 'Y') {

                    // Удаление файла
                    CFile::Delete(intval(${$arField["NAME"]}));
                    ${$arField["NAME"]} = '';

                } else if (is_array(${$arField["NAME"]})) {

                    // Удаление предыдущего файла если есть
                    $res = CFile::GetList(["ID"=>"desc"], ["MODULE_ID"=>"settings"]);
                    while($arFile = $res->GetNext()) {
                        if ($arFile["DESCRIPTION"] == $arField["NAME"]) {
                            CFile::Delete($arFile["ID"]);
                        }
                    }

                    $tmpRoot = CTempFile::GetAbsoluteRoot();
                    $tmp[$arField["NAME"]] = ${$arField["NAME"]};

                    if (isset(${$arField["NAME"] . '_del'}) && ${$arField["NAME"] . '_del'} == 'Y') {
                        $tmp[$arField["NAME"]]['del'] = 'Y';
                    }

                    $tmp[$arField["NAME"]]['description'] = $arField["NAME"];

                    $tmp[$arField["NAME"]]['tmp_name'] = $tmpRoot . $tmp[$arField["NAME"]]['tmp_name'];
                    $tmp[$arField["NAME"]]["MODULE_ID"] = "settings";

                    if (CFile::SaveForDB($tmp, $arField["NAME"], "settings")) {
                        ${$arField["NAME"]} = $tmp[$arField["NAME"]];
                    }
                }
            }

            COption::SetOptionString(
                "benequire.customsettings",
                $arField["NAME"],
                ${$arField["NAME"]}
            );
        }
...
$APPLICATION->SetTitle($arCustomPage["LANG"][LANGUAGE_ID]["PAGE_TITLE"]?$arCustomPage["LANG"][LANGUAGE_ID]["PAGE_TITLE"]:GetMessage("BENEQUIRE_CUSTOMSETTINGS_ADMIN_GCUSTOMSETTINGS_TITLE"));

// split data prepare and out
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");

if($module_mode==MODULE_DEMO)
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/install/trial/trial.php");
elseif($module_mode==MODULE_DEMO_EXPIRED)
	require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/install/trial/expired.php");

if($_REQUEST["mess"] == "ok")
  CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("BENEQUIRE_CUSTOMSETTINGS_ADMIN_GCUSTOMSETTINGS_DATA_SAVED"), "TYPE"=>"OK"));

if($settings_data_error)
  CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("BENEQUIRE_CUSTOMSETTINGS_ADMIN_GCUSTOMSETTINGS_DATA_FILE_ERROR"), "TYPE"=>"ERROR"));
elseif($settings_data_empty && $GKS_RIGHT=="W") {

?>
<?echo BeginNote();?>
	<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_ADMIN_GCUSTOMSETTINGS_NO_SETTINGS_NOTE",Array("#SETTINGS_URL#"=>"/bitrix/admin/settings.php?lang=".LANGUAGE_ID."&mid=benequire.customsettings"."&back_url_settings=".urlencode($APPLICATION->GetCurPageParam())))?>
<?echo EndNote();?>
<?
...
foreach($arCustomSettings as $tab_id => $arTab):
	...
	foreach($arTab["FIELDS"] as $arField):
		$val = COption::GetOptionString("benequire.customsettings", $arField["NAME"]);
	?>
		<tr>
			...
			<td valign="top" width="50%">
                    ...
                    <?elseif($arField["TYPE"]=="file"): ?>
                        <?= \Bitrix\Main\UI\FileInput::createInstance(array(
                            "name" => htmlspecialchars($arField["NAME"]),
                            "description" => false,
                            "upload" => true,
                            "allowUpload" => "F",
                            "medialib" => true,
                            ...
                            "maxCount" => 1,
                            "allowSort" => "N"
                            ))->show([htmlspecialchars($arField["NAME"]) => htmlspecialchars($val)]);
                        ?>
...
{
	if (confirm('<?echo AddSlashes(GetMessage("BENEQUIRE_CUSTOMSETTINGS_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
...
&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" <?if ($GKS_RIGHT<"S" || $module_mode==MODULE_DEMO_EXPIRED):?>disabled<?else:?>onClick="RestoreDefaults();"<?endif?> value="<?echo GetMessage("BENEQUIRE_CUSTOMSETTINGS_RESTORE_DEFAULTS")?>" />
...
if($GKS_RIGHT=="W") {
	echo BeginNote();
	?>
		<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_ADMIN_GCUSTOMSETTINGS_MODULE_SETTINGS_NOTE",Array("#SETTINGS_URL#"=>"/bitrix/admin/settings.php?lang=".LANGUAGE_ID."&mid=benequire.customsettings"."&back_url_settings=".urlencode($APPLICATION->GetCurPageParam())))?>
	<?
	echo EndNote();
}

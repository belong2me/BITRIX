<?
$module_id = "benequire.customsettings";
...
$GKS_RIGHT = $APPLICATION->GetGroupRight("benequire.customsettings");
...
$aTabs = array(
	array(
		"DIV" => "settings",
		"TAB" => GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_TAB_SETTINGS"),
		"TITLE" => GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_TAB_TITLE_SETTINGS"),
	),
	array(
		"DIV" => "menu",
		"TAB" => GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_TAB_MENU"),
		"TITLE" => GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_TAB_TITLE_MENU"),
	),
...
if($REQUEST_METHOD=="POST" && strlen($Update.$Apply.$RestoreDefaults)>0 && check_bitrix_sessid() && $GKS_RIGHT=="W") {
	...
	if (strlen($RestoreDefaults)>0)
	{
		...
		$handle = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data_default.php", "r");
		$settings_data=fread($handle, filesize($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data_default.php"));
		fclose($handle);

		$handle = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data.php", "w");
		fwrite($handle, $settings_data);
		fclose($handle);

		$handle = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/default_option_default.php", "r");
		$default_options_data=fread($handle, filesize($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/default_option_default.php"));
		fclose($handle);

		$handle = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/default_option.php", "w");
		...
		if(is_array($SETTINGS)) {
    		...
    		if($no_identifier_error) $arErrors[] = GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_ERROR_NO_IDENTIFIER");
			...
			if(count($arErrors)<=0) {
				...
				$default_options_data = "<?\n\$benequire_customsettings_default_option = ".var_export($default_options,true).";\n?>";
...
if(count($arErrors)<=0) {
	...
	$handle = fopen($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data.php", "r");
	$settings_data=fread($handle, filesize($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/benequire.customsettings/admin/settings_data.php"));
	fclose($handle);
...
/* this is strict copy of styles from /bitrix/modules/benequire.links/include.php */
#benequire_customsettings_data_source_params { padding-top: 8px }
table.benequire-links-dsparams { border: 1px solid rgb(208, 215, 216); border-collapse: collapse; width: 100% !important; margin: 0 0 8px 0 !important }
table.benequire-links-dsparams td { border: 1px solid rgb(208, 215, 216); padding: 5px 5px !important; background-color: white }
table.benequire-links-dsparams td.benequire-links-dsparams-col-name { text-align: right }
table.benequire-links-dsparams td.benequire-links-dsparams-col-input { text-align: left }
table.benequire-links-dsparams .benequire-links-dsparams-multiple { width: 190px }
table.benequire-links-dsparams .benequire-links-dsparams-multiple input { display: block }
...
<?if(CBeneQuire_CustomSettingsOptions::IsLinksInstalled()):?>
...
function gksShowLinksDataSourcePopup(name_prefix,hidden_inputs_container_id) {
	...
	$module_mode=CModule::IncludeModuleEx("benequire.links");

	if($module_mode==MODULE_DEMO_EXPIRED) {

		$popup .= GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_SETTINGS_LINK_MODULE_TRIAL_EXPIRED");

	} else {
	
		$arDataSourceList = CBeneQuire_LinksAdminTools::GetDataSourceList(true);

		$popup .= '<div style="text-align: center" id="benequire_customsettings_data_source_window">\n';
		$popup .= GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_SETTINGS_LINK_DATA_SOURCE").': ';
		$popup .= '<select name="--NAME--PREFIX--[DATA_SOURCE]" onchange="window.benequire_customsettings_dsparams_refresh(true,\\\'--NAME--PREFIX--\\\');" id="benequire_customsettings_data_source_select">\n';
		$popup .= '\t<option value=""></option>\n';
		foreach($arDataSourceList as $k=>$v):
			$popup .= '\t<option value="'.$k.'">'.$v.'</option>\n';
		endforeach;
		$popup .= '</select>\n';
		$popup .= '<div id="benequire_customsettings_data_source_params">\n';
		$popup .= '</div>';
		$popup .= '</div>';
	...
	if(typeof(window.benequire_customsettings_dsparams_popup)!="undefined" && window.benequire_customsettings_dsparams_popup.DIV)
		window.benequire_customsettings_dsparams_popup.DIV.parentNode.removeChild(window.benequire_customsettings_dsparams_popup.DIV);

	window.benequire_customsettings_dsparams_popup = new BX.CAdminDialog({
	    'title': '<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_SETTINGS_LINK_POPUP_HEADER")?>',
	    'content': '',
	    'draggable': true,
	    'resizable': true,
	    'buttons': []
	...
	var btn_save = {
	    ...
	    action: function () {
	    	...
	    	var obDSParamsContainer = document.getElementById('benequire_customsettings_data_source_window');
	    	
	    	var ar = window.benequire_customsettings_dsparams_getvalues(obDSParamsContainer,true);
	...
	window.benequire_customsettings_dsparams_popup.ClearButtons();
	window.benequire_customsettings_dsparams_popup.SetButtons([btn_save, BX.CAdminDialog.btnCancel]);
	window.benequire_customsettings_dsparams_popup.SetContent('<?=$popup?>'.replace(/--NAME--PREFIX--/g,name_prefix_tmp));
	
	//var wait = BX.showWait();
	//window.benequire_customsettings_dsparams_popup.DIV.style.zIndex+=10;
	//window.benequire_customsettings_dsparams_popup.CreateOverlay(parseInt(window.benequire_customsettings_dsparams_popup.DIV.style.zIndex)+10/* parseInt(BX.style(wait, 'z-index'))-1 */);
	//window.benequire_customsettings_dsparams_popup.OVERLAY.style.display = 'block';
	//window.benequire_customsettings_dsparams_popup.OVERLAY.className = 'bx-core-dialog-overlay';

	var ar = window.benequire_customsettings_dsparams_getvalues(obHiddenInputsContainer,false);
    var q = [];
    var data_source = '';
    ...
    var obContainer = document.getElementById("benequire_customsettings_data_source_params");
    if(!obContainer) return;
   	obContainer.innerHTML = "";

    var obSelect = document.getElementById("benequire_customsettings_data_source_select");
    if(!obSelect) return;
	obSelect.value = data_source;

    var url = '<?=BX_ROOT?>/admin/benequire_links_data_source_params.php?lang=<?=LANGUAGE_ID?>&NAME_PREFIX='+name_prefix_tmp+'&DATA_SOURCE='+data_source+'&MODULE_JS_PREFIX=benequire_customsettings';

    BX.ajax.post(url, q.join("&"), function(result){
    	obContainer.innerHTML = result;
   		//BX.closeWait(null, wait);
		window.benequire_customsettings_dsparams_popup.Show();
    });

}

// this is strict copy from /bitrix/modules/benequire.links/include.php
window.benequire_customsettings_dsparams_addvalues_change = function(obSelect) {
    var obAddValueInput = document.getElementById("benequire_customsettings_dspadd_"+obSelect.name);
...
window.benequire_customsettings_dsparams_addvalues_add = function(obButton)
{
...
window.benequire_customsettings_dsparams_refresh = function(bReset,name_prefix) {
    ...
    var url = '<?=BX_ROOT?>/admin/benequire_links_data_source_params.php?lang=<?=LANGUAGE_ID?>&NAME_PREFIX='+name_prefix+'&DATA_SOURCE='+data_source+'&MODULE_JS_PREFIX=benequire_customsettings';

    var ar = [];
    if(!bReset)
    	ar = window.benequire_customsettings_dsparams_getvalues(obContainer,true);
...
window.benequire_customsettings_dsparams_getvalues = function(obContainer,bDisable) {
...
foreach($arCustomSettings as $tab_id=>$tab_data) {
	echo CBeneQuire_CustomSettingsOptions::ShowTab($count,$tab_data,true);
	$count++;	
}
...
<a id="gks_tab_add" class="gcustomsettings-settings-tab-add" href="#" onclick="gksAddNewTab(); return false"><img src="/bitrix/images/benequire.customsettings/gcustomsettings_options_tab_icon_add.gif" width="30" height="15" border="0" />&nbsp;&nbsp;<span><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_SETTINGS_ADD_TAB")?></span></a>
	...
	?>
		<tr>
			<td valign="top" width="50%">
				<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU")?>
			</td>
			<td valign="top" width="50%">
				<select name="SETTINGS[PARENT_MENU]">
					<option value="global_menu_content"<?if($arCustomPage["PARENT_MENU"]=="global_menu_content"):?> selected="selected"<?endif?>><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_content")?></option>
					<option value="global_menu_services"<?if($arCustomPage["PARENT_MENU"]=="global_menu_services"):?> selected="selected"<?endif?>><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_services")?></option>
					<option value="global_menu_store"<?if($arCustomPage["PARENT_MENU"]=="global_menu_store"):?> selected="selected"<?endif?>><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_store")?></option>
					<option value="global_menu_statistics"<?if($arCustomPage["PARENT_MENU"]=="global_menu_statistics"):?> selected="selected"<?endif?>><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_statistics")?></option>
					<option value="global_menu_settings"<?if($arCustomPage["PARENT_MENU"]=="global_menu_settings"):?> selected="selected"<?endif?>><?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_settings")?></option>
				</select>
			</td>
		</tr>
		<tr>
			<td valign="top" width="50%">
				<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_SORT")?>
			</td>
			<td valign="top" width="50%">
		...
		<tr>
			<td valign="top" width="50%">
				<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_TEXT")?>
			</td>
			<td valign="top" width="50%">
		...
		<tr>
			<td valign="top" width="50%">
				<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_MENU_TITLE")?>
			</td>
			<td valign="top" width="50%">
				<table cellspacing="0" cellpadding="0" class="gcustomsettings-module-options">
					<?foreach($arLang as $lang_id => $lang):?>
		...
		<tr>
			<td valign="top" width="50%">
				<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_MENU_PARENT_PAGE_TITLE")?>
			</td>
			<td valign="top" width="50%">
				<table cellspacing="0" cellpadding="0" class="gcustomsettings-module-options">
					<?foreach($arLang as $lang_id => $lang):?>
...
<div style="display: none" id="new_gks_tab_template"><?=CBeneQuire_CustomSettingsOptions::ShowTab("--GKS_TAB_ID--",Array(),false)?></div>
<div style="display: none" id="new_gks_option_template"><?=CBeneQuire_CustomSettingsOptions::ShowOption("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"text","SIZE"=>"20"),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_text"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"text","SIZE"=>"20"),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_textarea"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"textarea","COLS"=>"20","ROWS"=>"5"),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_checkbox"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"checkbox"),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_select"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"select","VALUES"=>Array()),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_date"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"date"),false)?></div>
<div style="display: none" id="new_gks_option_custom_template_link"><?=CBeneQuire_CustomSettingsOptions::ShowOptionCustom("--GKS_TAB_ID--","--GKS_OPTION_ID--",Array("TYPE"=>"link"),false)?></div>

<div style="display: none" id="new_gks_selectvalue_template"><?=CBeneQuire_CustomSettingsOptions::ShowOptionSelectvalue("--GKS_TAB_ID--","--GKS_OPTION_ID--","--GKS_SELECTVALUE_ID--",Array(),false)?></div>
...
<?echo BeginNote();?>
	<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_NOTE")?>
<?echo EndNote();?>

<?echo BeginNote();?>
	<?=GetMessage("BENEQUIRE_CUSTOMSETTINGS_OPTIONS_NOTE2")?>
<?echo EndNote();?>
